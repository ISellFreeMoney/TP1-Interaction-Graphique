...
function createStairs() {
	...
	stepMesh.position.y = verticalStepHeight/2 + index * (stepThickness + verticalStepHeight);	// half of height: put it above ground plane
	stepMesh.position.z = 0 + (stepWidth-stepThickness * 3) * index ;			// centered at origin
	scene.add( stepMesh );
	...
	stepMesh.position.y = stepThickness/2 + verticalStepHeight + index * (stepThickness + verticalStepHeight);
	// Push step forward by half the depth, minus half the vertical step's thickness
	stepMesh.position.z = horizontalStepDepth/2 - stepHalfThickness  + (stepWidth-stepThickness * 3) * index;
	scene.add( stepMesh );
	}
}
